...
- Incidents are managed in **Azure Monitor**: the lab's action group raises the Azure Monitor alert;
  the agent investigates, annotates the thread with the RCA, and the alert auto-resolves on recovery.
- For Infrastructure-as-Code or application root causes, the agent opens a **GitHub pull request** with
  the fix, attaching the RCA.

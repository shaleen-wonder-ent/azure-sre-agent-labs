...
| `skills/pr-delivery/SKILL.md` | Single owner of GitHub PR creation for an IaC or code fix; applies after live mitigation | srectl skill apply |
...
and sets the incident filter and knowledge against the target agent. Incidents are managed in
**Azure Monitor** — the lab's action group raises Azure Monitor alerts that the agent investigates.
...
```
DIAGNOSE                DELIVER            REPORT (deliverable-only)
connectivity-triage  ·                  · rca-analysis
performance-invest.  ·   pr-delivery    · evidence-before-after
rbac-audit (report)  ·   (GitHub PR)    · recommendations-next-steps
                                        · zava-reporting (HTML report only)

INCIDENT LIFECYCLE
Azure Monitor  (alert fires → agent investigates → auto-resolves on recovery)
```

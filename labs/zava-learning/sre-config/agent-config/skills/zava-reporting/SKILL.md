...
(`recommendations-next-steps`), and the GitHub PR (`pr-delivery`), and renders them in the Zava
house style.
...
id/number, or plain text, and never wrapped in a table. The GitHub PR URL comes from `pr-delivery`;
the HTML report
...
2. **Email (HTML)** — the `zava-report-template` email layout: subject
   ...
   did, Recommended actions checklist, and links to the full report / PR. Write the
...
3. **Teams notification (Adaptive Card JSON)** — FactSet (Severity, Status, Duration, Users
   impacted), one-line root cause, top 3 next steps, and an OpenUrl button for the PR.
...
- File-creation self-audit: the HTML report (and any deck) was written into
  ...
  `[label](url)` — the HTML report (`/api/files/tmp/ThreadFiles/<threadId>/<file>`) and the PR —
  with no bare id/number and no plain-text
  placeholder.

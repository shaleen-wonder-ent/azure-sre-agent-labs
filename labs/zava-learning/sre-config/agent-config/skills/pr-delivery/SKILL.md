...
root-caused and the live mitigation is in place; the pull request it produces is referenced in the
incident report.
...
4. Commit with a conventional message: `fix(<area>): <symptom> — <root cause>` and a body that
   summarizes the RCA and links the incident.
5. Open the PR against `@@REPO@@`. PR body must include: symptom, root cause, the change,
   verification evidence (link the Before/After).
6. Capture the **PR URL** — the incident report references it.
...
## Verification
The PR is open against the correct repo/branch with a complete RCA-backed description;
CI (if any) is green. Report the PR URL.

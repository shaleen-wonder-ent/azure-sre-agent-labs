...
Record the request-path diagram and your diagnostic notes in the incident thread and the report.
...
- For an Infrastructure-as-Code root cause, the infra lives under `infra/` in `@@REPO@@`
  ...
  fix is delivered as a GitHub pull request by the `pr-delivery` skill.

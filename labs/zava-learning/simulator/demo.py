...
for the Azure Monitor alert and the SRE Agent's response — finally reporting
recovery. Inspired by the zavapl-lab intelligent simulator.
...
SCENARIOS = {
    "nsg": {
        ...
        "symptom_alert": "Zava-portal-unreachable",
...
def _load_dotenv() -> None:
    """Best-effort load of the gitignored sre-config/.env so values (e.g. GITHUB_REPO)
...
# ── Azure Monitor / SRE Agent polling ─────────────────────────────────
def _az(cmd: str):
    try:
...
# agent's data-plane API and correlate it to the incident we're tracking.
...
def resolve_incident_thread(cfg: dict, since: datetime,
                            expected_title: str | None = None):
    """Return the SRE Agent thread id for the incident under test, or None.

    Correlation, strongest first:
      1) exact match on the scenario's Azure Monitor alert title among threads created
         during this sim run;
      2) only when we have NO title to disambiguate, the newest incident thread in window.
    Step 1 pins the link to THIS scenario's thread; step 2 is a last resort that is
    ...
    # 1) Exact correlation to this scenario's alert title (newest matching).
    if expected_title:
...
def run_reset(cfg: dict, scenario: str | None = None) -> None:
    ...
    reset.ps1 reverts the infra source + redeploys clean; Azure Monitor alerts auto-resolve
    as the metrics return to healthy — giving a clean slate for the next demo."""
    if scenario is None:
        opts = "  ".join(f"[bold]{i}[/]={t}" for i, t in enumerate(RESET_TARGETS, 1))
        console.print(Panel(
            Text("Restore the platform and infra-as-code to a known-good baseline. Azure Monitor\n"
                 "alerts auto-resolve as metrics recover. Pick one scenario to reset, or 'all'.", style="white"),
    ...
    if scenario not in RESET_TARGETS:
        ...
        return
    console.print(f"\n[green]Restoring baseline[/] [bold]({scenario})[/] — reverts infra and redeploys "
                  "clean. This can take a few minutes.\n")
    ...
    if _stream_pwsh(cmd, "[green]Resetting the lab to baseline…[/]"):
        console.print("\n[bold green]✔ Baseline restored.[/] The platform is healthy — ready for "
                      "the next demo.\n")
    else:
...
def monitor(cfg: dict, sc: dict, auto_fix: bool):
    ...
    alert_fired = agent_seen = recovered = False
    ...
    # incident investigated + service recovered + the Azure Monitor alert auto-resolved. When a
    # live agent is wired up (the real lab), service-health recovery alone is NOT success — we
    # wait for the agent to engage and the alert to close. auto_fix is the dev stand-in (no live
    # agent), so it falls back to service recovery as the success signal.
    require_agent = bool(cfg.get("agent_name")) and not auto_fix
    completed_at = None            # when the success criteria were first met (for the tail hold)
    thread_url = None
    last_alert = last_agent = datetime.min.replace(tzinfo=timezone.utc)
    healthy_streak = 0
    ...
    def build():
        ...
        steps.add_row(Text("1. Outage detected by Azure Monitor, incident raised"), status_word(alert_fired))
        steps.add_row(Text("2. SRE Agent engaged & diagnosing"),
                      status_word(agent_seen, tracked=bool(cfg.get("agent_name"))))
        steps.add_row(Text("3. Service recovered"), status_word(recovered, pending="not yet"))
        steps.add_row(Text("4. SRE Agent closed out (alert auto-resolved)"),
    ...
    with Live(build(), console=console, refresh_per_second=4, screen=False) as live:
        for tick in range(900):  # ~15 min max
            ...
            # Detection signal: Azure Monitor alert. Log-query rules fire for the app/perf/query/
            # pool/secret/disk scenarios; the App Gateway UnhealthyHostCount metric alert
            # (Zava-portal-unreachable) fires for the nsg/appgw connectivity scenarios.
            if not alert_fired and (now - last_alert).total_seconds() >= 15:
                last_alert = now
                if poll_azmon_alert(sub, sc["symptom_alert"], sim_start):
                    alert_fired = True
                    timeline.add("Outage detected by Azure Monitor — the SRE Agent has an incident.", "yellow")

            # Agent-engaged signal: a correlated incident thread exists in the agent's data plane.
            if alert_fired and not agent_seen and cfg.get("agent_name") \
                ...
                if poll_agent_thread(sc["agent"], sim_start):
                    ...
                    if thread_url is None:
                        tid = resolve_incident_thread(cfg, sim_start,
                                                      expected_title=sc.get("symptom_alert"))
                        if tid:
            ...
            if not recovered and impacted_seen and healthy_streak >= 4 \
                ...
                # Azure Monitor auto-resolves the alert once the metric returns to healthy.
                if agent_seen and not agent_resolved:
                    agent_resolved = True
                    timeline.add("Azure Monitor alert auto-resolved — incident closed.", "green")
            ...
            live.update(build())
            # Success = the SRE Agent CLOSED the loop (service recovered + alert auto-resolved).
    ...
    success = agent_resolved if require_agent else recovered
    _report(sc, alert_fired, agent_seen, recovered, agent_resolved,
            require_agent, success, thread_url)


def _report(sc, alert_fired, agent_seen, recovered, agent_resolved,
            require_agent, success, thread_url=None):
    lines = [
        ("Symptom alert fired", alert_fired),
        ("SRE Agent engaged", agent_seen),
        ("Service recovered", recovered),
        ("Azure Monitor alert auto-resolved", agent_resolved),
    ]
    ...
    if success and require_agent:
        console.print("[bold green]✔ Demo successful — the SRE Agent diagnosed and remediated the "
                      "incident, and the Azure Monitor alert auto-resolved.[/]")
    elif success:
        console.print("[green]✔ Service recovered (dev stand-in mode — no live SRE Agent in this "
                      "run).[/]")
    elif require_agent and recovered and not agent_resolved:
        console.print("[yellow]⚠ Service recovered, but the incident has not closed out yet — "
                      "check the agent thread.[/]")
    else:

...
Produces docs/Zava-Learning-Architecture.pptx:
  ...
  * the incident lifecycle (fault -> Azure Monitor alert -> SRE Agent -> IaC PR)
...
add_text(s, 0.92, 6.75, 12, 0.4, "Blue/White Azure theme  •  App Gateway · Container Apps · PostgreSQL · Key Vault · SRE Agent · Azure Monitor · Scheduled Tasks",
...
add_box(s, 2.65, 5.5, 1.85, 1.05, "Incident filter\n(response plan)", fill=NAVY, line=CYAN, size=12, bold=True, color=WHITE)
...
add_box(s, 0.55, 5.0, 12.2, 1.7,
        "Two detection paths:  log-based Azure Monitor alerts catch the latency / error scenarios, while the App\n"
        "Gateway health metric alert (Zava-portal-unreachable) catches connectivity blackholes that emit no logs.  The simulator\n"
...
add_text(s, 0.7, 3.5, 12.0, 1.5,
         ...
         "Then, in order:   rca-analysis → evidence-before-after → recommendations-next-steps →\n"
         "                          pr-delivery → zava-reporting\n"
...
panel(s, 0.55, 4.55, 6.0, 2.25, "Investigation & evidence", [
    "•  Azure Monitor incident — investigated & auto-resolved",
...
panel(s, 6.75, 4.55, 6.0, 2.25, "Durable fix & hand-off", [
    ...
    "•  Symptom-only in-thread summary (cause never assumed)",
], body_size=11)
...
RESPONSE = {
    "nsg": {
        ...
        "trigger": "App Gateway health alert\n(Zava-portal-unreachable):\n'quiz launches failing'.",
...
def response_slide(key):
    ...
    add_box(s, 0.5, 5.9, 12.3, 0.78, "Artifacts", fill=LIGHT1, line=AZURE, size=11.5, bold=True,
            ...
            sub="live mitigation (" + d["mitig"] + ")  \u00b7  GitHub PR (" + d["durable"] +
                ")  \u00b7  RCA  \u00b7  before/after evidence  \u00b7  HTML report  \u00b7  alert auto-resolved",
...
add_box(s, 0.55, 1.4, 12.2, 1.5,
        "Students → Application Gateway → Container Apps → PostgreSQL + Key Vault.  Azure Monitor alerts\n"
        "reach the SRE Agent; it diagnoses against this exact topology, mitigates live,\n"

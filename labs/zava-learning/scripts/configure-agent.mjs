...
//   MCP connector -> tools -> skills -> response plan (Azure Monitor) -> knowledge base
//
...
// Secrets are read from sre-config/.env (gitignored): GITHUB_REPO.
...
// them resolve, then apply each skill, then the custom
...
// Resolve the agent's data-plane endpoint once; used by srectl (skills).
...
// ---- 3. Incident response plan / trigger (symptom-keyed, autonomous) ------
// Created as a Microsoft.App/agents/incidentFilters child resource over ARM.
// Azure Monitor is the incident platform: Azure Monitor alerts raised by the lab's
// action group become incidents the agent investigates. Symptom-only: titleContains
// "Zava" routes every student-facing Zava incident to the custom zava-incident-responder
// agent, which scopes the skill menu (allowedSkills + system skills) and prescribes the
// ordered runbook (triage -> mitigate -> RCA -> evidence -> recommendations -> PR ->
// report) via its instructions + critic-on-handoff gate. The cause-level split (NSG vs
// LB vs AppGW vs app) is decided inside the skills from telemetry, never by alert name.
const filterSpec = {

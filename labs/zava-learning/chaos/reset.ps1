...
.DESCRIPTION
  Calls the same fix logic used by each lane's fix-*.ps1 script. Once the faults are
  reverted, the Azure Monitor alerts auto-resolve as the underlying metrics return to
  healthy. The -Redeploy switch is kept
...
Write-Host "[reset] Done. Source and live state restored to baseline. Azure Monitor alerts auto-resolve on recovery." -ForegroundColor Green

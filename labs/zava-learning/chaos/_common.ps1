...
function Invoke-PgSql {
  ...
  # path silently no-op'd the DB faults while the alert still fired. psycopg2 connects over the
  ...
  # so a break that did not land aborts before the fault is considered live.
  $py = @'
import os, sys
try:
    import psycopg2
  ...
  for ($i = 1; $i -le $MaxAttempts; $i++) {
    ...
    if ($res.ok) {
      ...
      if ($consecutive -ge $Failures) {
        Write-Host "[synthetic] $Failures consecutive failures confirmed. App Gateway reports the backend" -ForegroundColor Magenta
        Write-Host "            unhealthy, so the Azure Monitor metric alert 'Zava-portal-unreachable' fires and" -ForegroundColor Magenta
        Write-Host "            the SRE Agent investigates. This probe is verification-only; it does not page." -ForegroundColor Magenta
        return @{ outage = $true; detail = $res.detail }
      }
    }

...
For a modular blueprint covering outage RCA, network and VM diagnostics, SQL Managed Instance,
availability, change intelligence, Entra authentication, capacity, deployment, cost, security, and
multi-subscription health, see the [Enterprise Operations Scenario Pack](enterprise-operations/README.md).
It includes the target architecture, permissions, artifact tree, phased build plan, validation gates,
the [detailed implementation and operator guide](enterprise-operations/docs/use-case-implementation-guide.md),
and [copy-ready prompts for all 13 scenarios](enterprise-operations/prompts/scenario-prompts.md).

The pack is an implementation blueprint: it clearly distinguishes scenarios that run on existing
labs from optional modules that still require new infrastructure.
